...
In the same folder, create a new file `index.js` with the following content.
...
Update the `Dockerfile` in the `graphql` folder so that it looks like the one below.
...
In the file `docker-compose.yml`, add the custom plugin in the `graphql` service command.
...
Finally rebuild the GraphQL container.

```
$ doker-compose build graphql
```
